...
df_avg = CSV.read(joinpath(@__DIR__,  "..", "model_structure_$(structure_best)", "metrics_train_avg.csv"), DataFrame)
idx_min = findfirst(x -> x == minimum(df_avg.MSE_test), df_avg.MSE_test)
NN_model = load(joinpath(@__DIR__, "..", "model_structure_$structure_best", "neurons_$(df_avg.neurons[idx_min])","NN_model.jld2"))["NN_model"]
nn, p_init, st = initialize_neural_network(saveat = joinpath(@__DIR__), input = 4, neurons = [df_avg.neurons[idx_min]], output = 4, activation = (tanh_fast, softplus))
...
df1 = CSV.read(joinpath(@__DIR__, "..", "model_structure_3", "neurons_5", "convergence.csv"), DataFrame) # Correponds to model 6, using 3 neurons a 2 layer 
df2 = CSV.read(joinpath(@__DIR__, "..", "model_structure_8", "neurons_3_2", "convergence_2.csv"), DataFrame) # Correponds to model 7, using 8 neurons a 1 layer
df3 = CSV.read(joinpath(@__DIR__, "..", "model_structure_15", "neurons_7_2", "convergence_2.csv"), DataFrame) # Correponds to model 9, using 6 neurons a 2 layer
df = (df1, df2, df3)
labels = ["HM3", "HM7", "HM10"]
p1 = plot()
for i=1:3 #i=1
