...
include(joinpath(@__DIR__, "reaction_base_hm12.jl"))
...
qmax = [12., 10., 40., 6.]
kl = [0.25, 0.25, 0.25, 0.25]


for l in neurons_test #l=10
	...
	# Inital weights - vary if using multiple simulatenous networks in one model and/or mechanistic parameters
	global u0 = ComponentArray(kl = kl, qmax = qmax, p1 = p_init_pretrain)
	...
	evaluate_metrics_training(@__DIR__, l, 2, 12)
...
#nn, p_init, st = initialize_neural_network(saveat = @__DIR__, input = 4, neurons = [df_avg.neurons[idx_min],df_avg.neurons[idx_min]], output = 8, activation = (tanh_fast, tanh_fast, softplus))
