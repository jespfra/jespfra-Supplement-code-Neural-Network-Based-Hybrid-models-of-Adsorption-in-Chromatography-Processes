...
function (problem!::hybrid_model)(RHS, x, p, t)
	...
	@inbounds for h = 1:nColumns 
		...
		denom = 1 .+ (@view nn(input, p, st)[1][5, 1:end]) .* 7.5 .* cp1 .* c_scale .+ (@view nn(input, p, st)[1][6, 1:end]) .* 7.5 .* cp2 .* c_scale .+ (@view nn(input, p, st)[1][7, 1:end]) .* 7.5 .* cp3 .* c_scale .+ (@view nn(input, p, st)[1][8, 1:end]) .* 7.5 .* cp4 .* c_scale
		
		for j = 1:columns[h].nComp
			RHS_q[1 + (j-1) * columns[h].bindStride : columns[h].bindStride + (j-1) * columns[h].bindStride] .= (@view nn(input, p, st)[1][j, 1:end]) .* 50. .* x[1 + columns[h].adsStride + columns[h].bindStride * (j-1) + idx_units[h] : columns[h].adsStride + columns[h].bindStride * j + idx_units[h]] ./ denom .- x[1 + columns[h].adsStride + columns[h].bindStride * columns[h].nComp + idx_units[h] + (j-1) * columns[h].bindStride : columns[h].adsStride + columns[h].bindStride * columns[h].nComp + columns[h].bindStride * j + idx_units[h]]
		end
...
function (problem!::hybrid_model_reac)(RHS, x, param, t)
	...
	@inbounds for h = 1:nColumns 
		...
		denom = 1. .+ (@view nn(input, param, st)[1][5, 1:end]) .* 7.5 .* cp1 .* c_scale .+ (@view nn(input, param, st)[1][6, 1:end]) .* 7.5 .* cp2 .* c_scale .+ (@view nn(input, param, st)[1][7, 1:end]) .* 7.5 .* cp3 .* c_scale .+ (@view nn(input, param, st)[1][8, 1:end]) .* 7.5 .* cp4 .* c_scale
		
		for j = 1:columns[h].nComp
			RHS_q[1 + (j-1) * columns[h].bindStride : columns[h].bindStride + (j-1) * columns[h].bindStride] .= (@view nn(input, param, st)[1][j, 1:end]) .* 50. .* x[1 + columns[h].adsStride + columns[h].bindStride * (j-1) + idx_units[h] : columns[h].adsStride + columns[h].bindStride * j + idx_units[h]] ./ denom .- x[1 + columns[h].adsStride + columns[h].bindStride * columns[h].nComp + idx_units[h] + (j-1) * columns[h].bindStride : columns[h].adsStride + columns[h].bindStride * columns[h].nComp + columns[h].bindStride * j + idx_units[h]]
		end
        ...
        # Compute reaction term directly from stationary phase 
		qq = x[1 + columns[h].adsStride + columns[h].bindStride * columns[h].nComp + idx_units[h] : columns[h].adsStride + columns[h].bindStride * columns[h].nComp * 2 + idx_units[h]]
		compute_reaction(RHS_q, qq, columns[h].reaction_solid, columns[h].eps_c, columns[h].nComp, columns[h].bindStride, t, input, param)
...
function init_conditions_function(input, params, st, h)
	denom = 1.0 .+ nn(input, params, st)[1][5,:] .* 7.5 .* input[1,:] .* c_scale_data[2] .+ nn(input, params, st)[1][6,:] .* 7.5 .* input[2,:] .* c_scale_data[2] .+ nn(input, params, st)[1][7,:] .* 7.5 .* input[3,:] .* c_scale_data[2] .+ nn(input, params, st)[1][8,:] .* 7.5 .* input[4,:] .* c_scale_data[2]
	return nn(input, params, st)[1][h,:] .* 50 .* input[h,:] .* c_scale_data[2] ./ denom
end
