...
for l in neurons_test #l=4
	println("Training at $l neurons")

	# Set up neural network - input and output depends on model structure
	global nn, p_init, st = initialize_neural_network(saveat = joinpath(@__DIR__, "neurons_$(l)_2"), input = 4, neurons = [l,l], output = 8, activation = (tanh_fast, tanh_fast, softplus))
	...
	evaluate_metrics_training(@__DIR__, l, 2, 4)

end
# Neurons tested
#neurons_test = collect(2:1:8)
