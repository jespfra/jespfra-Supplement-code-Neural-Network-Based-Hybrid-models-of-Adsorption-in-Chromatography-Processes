...
include(joinpath(@__DIR__, "reaction_base_hm4.jl"))
...
for l in neurons_test #l=2
	...
	global nn, p_init, st = initialize_neural_network(saveat = joinpath(@__DIR__, "neurons_$l"), input = 4, neurons = l, output = 8, activation = (tanh_fast, softplus))

	# Inital weights - vary if using multiple simulatenous networks in one model and/or mechanistic parameters
	global u0_pretrain = ComponentArray(p_init) #u0_pretrain = u0

	# pretraing model
	global u0 = opti_nn_pretraining()
	...
	evaluate_metrics_training(@__DIR__, l, 1, 4)
...
#nn, p_init, st = initialize_neural_network(saveat = @__DIR__, input = 4, neurons = df_avg.neurons[idx_min], output = 8, activation = (tanh_fast, softplus))
...
# idx = 9:14
# denom = 1.0 .+ nn(input_pretrain, param, st)[1][5,idx] .* 7.5 .* input_pretrain[1,idx] .* c_scale_data[2] .+ nn(input_pretrain, param, st)[1][6,idx] .* 7.5 .* input_pretrain[2,idx] .* c_scale_data[2] .+ nn(input_pretrain, param, st)[1][7,idx] .* 7.5 .* input_pretrain[3,idx] .* c_scale_data[2] .+ nn(input_pretrain, param, st)[1][8,idx] .* 7.5 .* input_pretrain[4,idx] .* c_scale_data[2]
# scatter(input_pretrain[1,idx] .* c_scale_data[2], nn(input_pretrain, param, st)[1][1,idx] .* 50 .* input_pretrain[1, idx] .* c_scale_data[2] ./ denom)
# scatter!(input_pretrain[1,idx] .* c_scale_data[2], output_pretrain[idx,1])

# scatter(input_pretrain[4,idx] .* c_scale_data[2], nn(input_pretrain, param, st)[1][4,idx] .* 50 .* input_pretrain[4, idx] .* c_scale_data[2] ./ denom)
# scatter!(input_pretrain[4,idx] .* c_scale_data[2], output_pretrain[idx,4])
