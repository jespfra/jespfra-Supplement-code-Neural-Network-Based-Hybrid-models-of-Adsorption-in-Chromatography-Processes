...
mutable struct reaction_HM4 <: reactionBase
	...
	function reaction_HM4(; k1::Float64, Keq::Float64, K::Vector{Float64}, stoich::Vector{Float64}, density::Float64, Vm::Vector{Float64}, Tmodel, nComp, compStride, T, nn, nn_par, st)
...
function compute_reaction(RHS_rate, cp_reaction, reaction::reaction_HM4, eps_, nComp, compStride, t, input, param)
	"""
	...
	for j=1:compStride
		...
		rate[j] *= reaction.nn(input[:,j], param, reaction.st)[1][5, 1] .* 7.5 * reaction.nn(input[:,j], param, reaction.st)[1][6, 1] .* 7.5 / (1 +  reaction.nn(input[:,j], param, reaction.st)[1][5, 1] .* 7.5 * y_mole[j,1] * gammas[1]  +  reaction.nn(input[:,j], param, reaction.st)[1][6, 1] .* 7.5 * y_mole[j,2] * gammas[2] +  reaction.nn(input[:,j], param, reaction.st)[1][7, 1] .* 7.5 * y_mole[j,3] * gammas[3]  +  reaction.nn(input[:,j], param, reaction.st)[1][8, 1] .* 7.5 * y_mole[j,4] * gammas[4])
	end
