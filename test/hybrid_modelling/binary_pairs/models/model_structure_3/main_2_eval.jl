...
for l in neurons_test #l=2
	...
	global nn, p_init, st = initialize_neural_network(saveat = joinpath(@__DIR__, "neurons_$(l)_2"), input = 4, neurons = [l,l], output = 4, activation = (tanh_fast, tanh_fast, softplus))
	global nn2, p_init2, st2 = initialize_neural_network(saveat = joinpath(@__DIR__, "neurons_$(l)_2"), input = 4, neurons = [l,l], output = 1, activation = (tanh_fast, tanh_fast, softplus))
	...
	# Inital weights - vary if using multiple simulatenous networks in one model and/or mechanistic parameters
	global u0 =  ComponentArray(p1 = p_init_pretrain, p2 = p_init2)
	...
	# save model 
	#@save joinpath(@__DIR__, "neurons_$(l)_2","NN_model_2.jld2") NN_model
	global NN_model = load(joinpath(@__DIR__, "neurons_$(l)_2","NN_model_2.jld2"))["NN_model"]
	
	# Run model on training, save training results and plots, run on test data 
	evaluate_metrics_training(@__DIR__, l, 2, 3)
...
NN_model = load(joinpath(@__DIR__, "neurons_$(df_avg.neurons[idx_min])_2","NN_model_2.jld2"))["NN_model"]
