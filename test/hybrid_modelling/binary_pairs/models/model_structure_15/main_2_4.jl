...
for l in neurons_test #l=10
	...
	# Inital weights - vary if using multiple simulatenous networks in one model and/or mechanistic parameters
	global u0 = ComponentArray(kl = kl, Keq = Keq, p1 = p_init_pretrain)
	...
	evaluate_metrics_training(@__DIR__, l, 2, 15)
...
#nn, p_init, st = initialize_neural_network(saveat = @__DIR__, input = 4, neurons = df_avg.neurons[idx_min], output = 8, activation = (tanh_fast, tanh_fast, softplus))
