...
function (problem!::hybrid_model)(RHS, x, p, t)
	...
	@inbounds for h = 1:nColumns 
		...
		for j = 1:columns[h].nComp
			RHS_q[1 + (j-1) * columns[h].bindStride : columns[h].bindStride + (j-1) * columns[h].bindStride] .= kl .* (qmax[j] .* nn(input, p_NN1, st)[1][j, 1:end] .* 7.5 .* x[1 + columns[h].adsStride + columns[h].bindStride * (j-1) + idx_units[h] : columns[h].adsStride + columns[h].bindStride * j + idx_units[h]] ./ denom .- x[1 + columns[h].adsStride + columns[h].bindStride * columns[h].nComp + idx_units[h] + (j-1) * columns[h].bindStride : columns[h].adsStride + columns[h].bindStride * columns[h].nComp + columns[h].bindStride * j + idx_units[h]])
		end
