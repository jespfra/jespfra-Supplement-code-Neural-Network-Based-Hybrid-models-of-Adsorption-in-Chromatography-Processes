...
function (problem!::hybrid_model)(RHS, x, p, t)
	...
	@inbounds for h = 1:nColumns 
		...
		denom = 1 .+ (@view nn(input, p_NN1, st)[1][1, 1:end]) .* 7.5  .* cp1 .* c_scale .+ (@view nn(input, p_NN1, st)[1][2, 1:end]) .* 7.5.* cp2 .* c_scale .+ (@view nn(input, p_NN1, st)[1][3, 1:end]) .* 7.5 .* cp3 .* c_scale .+ (@view nn(input, p_NN1, st)[1][4, 1:end]) .* 7.5 .* cp4 .* c_scale
		
		
		#Components
		for j = 1:columns[h].nComp
			RHS_q[1 + (j-1) * columns[h].bindStride : columns[h].bindStride + (j-1) * columns[h].bindStride] .= kl[j] .* (qmax[j] .* nn(input, p_NN1, st)[1][j, 1:end] .* 7.5 .* x[1 + columns[h].adsStride + columns[h].bindStride * (j-1) + idx_units[h] : columns[h].adsStride + columns[h].bindStride * j + idx_units[h]] ./ denom .- x[1 + columns[h].adsStride + columns[h].bindStride * columns[h].nComp + idx_units[h] + (j-1) * columns[h].bindStride : columns[h].adsStride + columns[h].bindStride * columns[h].nComp + columns[h].bindStride * j + idx_units[h]])
		end
...
# Define hybrid RHS 
function (problem!::hybrid_model_reac)(RHS, x, p, t)
	"""
		ODE problem formulation for hybrid models.
...
function pretrain_function(input_pretrain, param, st, h) #param = u0
	denom = 1.0 .+ nn(input_pretrain, param, st)[1][1,:] .* 7.5 .* input_pretrain[1,:] .* c_scale_data[2] .+ nn(input_pretrain, param, st)[1][2,:] .* 7.5 .* input_pretrain[2,:] .* c_scale_data[2] .+ nn(input_pretrain, param, st)[1][3,:] .* 7.5 .* input_pretrain[3,:] .* c_scale_data[2] .+ nn(input_pretrain, param, st)[1][4,:] .* 7.5 .* input_pretrain[4,:] .* c_scale_data[2]
	return qmax[h] .* nn(input_pretrain, param, st)[1][h,:] .* 7.5 .* input_pretrain[h,:] .* c_scale_data[2] ./ denom
end
function init_conditions_function(input, params, st, h)
	denom = 1.0 .+ nn(input, params.p1, st)[1][1,:] .* 7.5 .* input[1,:] .* c_scale_data[2] .+ nn(input, params.p1, st)[1][2,:] .* 7.5 .* input[2,:] .* c_scale_data[2] .+ nn(input, params.p1, st)[1][3,:] .* 7.5 .* input[3,:] .* c_scale_data[2] .+ nn(input, params.p1, st)[1][4,:] .* 7.5 .* input[4,:] .* c_scale_data[2]
	return params.qmax[h] .*nn(input, params.p1, st)[1][h,:] .* 7.5 .* input[h,:] .* c_scale_data[2] ./ denom
end