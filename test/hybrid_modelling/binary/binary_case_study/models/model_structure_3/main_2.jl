...
for l in neurons_test #l=2
	...
	global nn2, p_init2, st2 = initialize_neural_network(saveat = joinpath(@__DIR__, "neurons_$(l)_2"), input = 2, neurons = [l,l], output = 1, activation = (tanh_fast, tanh_fast, softplus))
...
nn, p_init, st = initialize_neural_network(saveat = @__DIR__, input = 2, neurons = [df_avg.neurons[idx_min],df_avg.neurons[idx_min]], output = 2, activation = (tanh_fast, tanh_fast, softplus))
