...
function (problem!::hybrid_model)(RHS, x, p, t)
	...
	@inbounds for h = 1:nColumns 
        ...
        input = [cp1 cp2]'
		denom = 1 .+ (@view nn(input, p, st)[1][3, 1:end]) .* 7.5 .* cp1 .* c_scale .+ (@view nn(input, p, st)[1][4, 1:end]) .* 7.5 .* cp2 .* c_scale # to scale the NN, 7.5 is used.
		RHS_q[1 : columns[h].bindStride] .= ((@view nn(input, p, st)[1][1, 1:end]) .* 50. .*cp1 .* c_scale ./ denom .- q1) # to scale the NN, 50 is used.
		RHS_q[1 + columns[h].bindStride : columns[h].bindStride * 2] .= ((@view nn(input, p, st)[1][2, 1:end]) .* 50. .* cp2 .* c_scale ./ denom .- q2)
...
function pretrain_function(input_pretrain, params, st, h)
	denom = 1.0 .+ nn(input_pretrain, params, st)[1][3,:] .* 7.5 .* input_pretrain[1,:] .* c_scale_data .+ nn(input_pretrain, params, st)[1][4,:] .* 7.5 .* input_pretrain[2,:] .* c_scale_data
	return nn(input_pretrain, params, st)[1][h,:] .* 50 .* input_pretrain[h,:] .* c_scale_data ./ denom
end

function init_conditions_function(input, params, st, h)
	denom = 1.0 .+ nn(input, params, st)[1][3,:] .* 7.5 .* input[1,:] .* c_scale_data .+ nn(input, params, st)[1][4,:] .* 7.5 .* input[2,:] .* c_scale_data
	return nn(input, params, st)[1][h,:] .* 50 .* input[h,:] .* c_scale_data ./ denom
end
