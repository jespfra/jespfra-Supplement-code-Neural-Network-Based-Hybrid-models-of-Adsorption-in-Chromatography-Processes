...
function (problem!::hybrid_model)(RHS, x, p, t)
	...
	@inbounds for h = 1:nColumns 
        ...
        input = [cp1 cp2]'
		denom = 1 .+ (@view nn(input, p_NN1, st)[1][1, 1:end]) .* 7.5 .* cp1 .* c_scale .+ (@view nn(input, p_NN1, st)[1][2, 1:end]) .* 7.5 .* cp2 .* c_scale #nn scaling factor of 7.5
		RHS_q[1 : columns[h].bindStride] .= kl .* (qmax[1] .* (@view nn(input, p_NN1, st)[1][1, 1:end]) .* 7.5 .*cp1 .* c_scale ./ denom .- q1)
		RHS_q[1 + columns[h].bindStride : columns[h].bindStride * 2] .= kl .* (qmax[2] .* (@view nn(input, p_NN1, st)[1][2, 1:end]) .* 7.5 .* cp2 .* c_scale ./ denom .- q2)
