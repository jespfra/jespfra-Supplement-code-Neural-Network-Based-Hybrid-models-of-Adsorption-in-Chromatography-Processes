...
for l in neurons_test #l=2
	...
	global nn, p_init, st = initialize_neural_network(saveat = joinpath(@__DIR__, "neurons_$(l)_2"), input = 2, neurons = [l,l], output = 2, activation = (tanh_fast, tanh_fast, identity))
	...
	evaluate_metrics_validation(@__DIR__, 1, l, 2, 0)

end

neurons_test = collect(2:1:10)
# Summmarize results for all tests and save as csv file 
evaluate_metrics_validation_all(@__DIR__, 2, 0)
