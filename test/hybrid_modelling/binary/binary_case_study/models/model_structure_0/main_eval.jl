...
for l in neurons_test #l=2
	...
	global nn, p_init, st = initialize_neural_network(saveat = joinpath(@__DIR__, "neurons_$l"), input = 2, neurons = l, output = 2, activation = (tanh_fast, identity))
	...
	evaluate_metrics_validation(@__DIR__, 1, l, 1, 0)
		
	 
end

# Summmarize results for all tests and save as csv file 
evaluate_metrics_validation_all(@__DIR__, 1, 0)
...
nn, p_init, st = initialize_neural_network(saveat = @__DIR__, input = 2, neurons = df_avg.neurons[idx_min], output = 2, activation = (tanh_fast, softplus))
