...
function (problem!::hybrid_model)(RHS, x, p, t)
	...
	@inbounds for h = 1:nColumns 
		...
		RHS_q[1 : columns[h].bindStride] .= kl .* ((@view nn(input, p_NN1, st)[1][1, 1:end]) .*50 .* cp1 .* c_scale ./ denom .- q1) #multiply w. 50 because the NN cannot reach those values by itself
