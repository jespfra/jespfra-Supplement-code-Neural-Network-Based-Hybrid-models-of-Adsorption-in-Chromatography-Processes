...
function (problem!::hybrid_model)(RHS, x, p, t)
	...
	@inbounds for h = 1:nColumns 
        ...
        input = [cp1 cp2]'
		RHS_q[1 : columns[h].bindStride] .= kl .* ((@view nn(input, p_NN1, st)[1][1, 1:end]) - q1)
		RHS_q[1 + columns[h].bindStride : columns[h].bindStride * 2] .= kl .* ((@view nn(input, p_NN1, st)[1][2, 1:end]) - q2)
