...
function (problem!::hybrid_model)(RHS, x, p, t)
	...
	@inbounds for h = 1:nColumns 
        ...
        input = [cp1 cp2]'
		denom = 1 .+ (@view nn(input, p_NN1, st)[1][3, 1:end]) .* 7.5 .* cp1 .* c_scale .+ (@view nn(input, p_NN1, st)[1][4, 1:end]) .* 7.5 .* cp2 .* c_scale
		RHS_q[1 : columns[h].bindStride] .= kl .* ((@view nn(input, p_NN1, st)[1][1, 1:end]) .* 50 .*cp1 .* c_scale ./ denom .- q1)
		RHS_q[1 + columns[h].bindStride : columns[h].bindStride * 2] .= kl .* ((@view nn(input, p_NN1, st)[1][2, 1:end]) .* 50 .* cp2 .* c_scale ./ denom .- q2)
...
function init_conditions_function(input, params, st, h)
	denom = 1.0 .+ nn(input, params.p1, st)[1][3,:] .* 7.5 .* input[1,:] .* c_scale_data .+ nn(input, params.p1, st)[1][4,:] .* 7.5 .* input[2,:] .* c_scale_data
