...
function pretrain_function(input_pretrain, params, st, h)
	denom = 1.0 .+ params.ka[1] .* input_pretrain[1,:] .* c_scale_data .+ params.ka[2] .* input_pretrain[2,:] .* c_scale_data
	return params.qmax[h] .* params.ka[h] .* input_pretrain[h,:] .* c_scale_data ./ denom
end

function init_conditions_function(input, params, st, h)
	denom = 1.0 .+ params.ka[1] .* input[1,:] .* c_scale_data .+ params.ka[2] .* input[2,:] .* c_scale_data
	return params.qmax[h] .* params.ka[h] .* input[h,:] .* c_scale_data ./ denom
end