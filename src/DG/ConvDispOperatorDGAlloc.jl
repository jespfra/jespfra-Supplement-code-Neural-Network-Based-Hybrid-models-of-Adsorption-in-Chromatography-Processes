...
module ConvDispOperatorDGAlloc
    ...
    @inline function residualImpl(y, idx, _strideNode,_strideCell,_nPoints,_nNodes, _nCells,_deltaZ, _polyDeg, _invWeights, _polyDerM,_invMM, u, d_ax, cIn,_exactInt = 1) #Convection Dispersion operator
        ...
        Dh = volumeIntegral(_h, idx, _nCells, _nNodes, _polyDerM)
    ...
    @inline function volumeIntegraly(state, idx, _nCells, _nNodes, _polyDerM)
        ...
        stateDer = zeros(eltype(state), length(idx))
    ...
    # calculates the volume Integral of the auxiliary equation
    @inline function volumeIntegral(state, idx, _nCells::Int64, _nNodes::Int64, _polyDerM)
        #Determines the Dc for all the cells in the grid
        stateDer = zeros(eltype(state), length(idx))
