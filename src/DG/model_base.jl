...
mutable struct LRM <: ModelBase
	...
	bind::bindingBase
	checkReactions::ReactionState
	reaction_liquid::reactionBase
	reaction_particle::reactionBase
	reaction_solid::reactionBase
	...
	function LRM(; nComp, colLength, d_ax, eps_c, c0 = 0.0, cp0 = -1, q0 = 0, polyDeg=4, nCells=8, exactInt=1)
		...
		# Default reactions - assumes no reactions 
		checkReactions = NoReactions()
		reaction_liquid = no_reaction()
		reaction_particle = no_reaction()
		reaction_solid = no_reaction()
		
		# The new commando must match the order of the elements in the struct!
		new(nComp, colLength, d_ax, eps_c, c0, cp0, q0, cIn, exactInt, polyDeg, nCells, ConvDispOpInstance, bindStride, adsStride, unitStride, idx, Fc, Fjac, cpp, RHS_q, qq, RHS, solution_outlet, solution_times, bind, checkReactions, reaction_liquid, reaction_particle, reaction_solid)
	end
end
...
mutable struct LRMP <: ModelBase
	...
	function LRMP(; nComp, colLength, d_ax, eps_c, eps_p, kf, Rp, c0 = 0.0, cp0 = -1, q0 = 0, polyDeg=4, nCells=8, exactInt=1)
		...
		# The new commando must match the order of the elements in the struct!
		new(nComp, colLength, d_ax, eps_c, eps_p, kf, Rp, c0, cp0, q0, cIn, exactInt, polyDeg, nCells, ConvDispOpInstance, bindStride, adsStride, unitStride, idx, idx_p, Fc, Fp, Fjac, cpp, RHS_q, qq, RHS, solution_outlet, solution_times, bind, checkReactions, reaction_liquid, reaction_particle, reaction_solid)
	end
end
...
mutable struct GRM <: ModelBase
	...
	function GRM(; nComp, colLength, d_ax, eps_c, eps_p, kf, Rp, Dp, Rc=0.0, c0 = 0.0, cp0 = -1, q0 = 0, polyDeg=4, polyDegPore=4, nCells=8, exactInt=1)
		...
		# The new commando must match the order of the elements in the struct!
		new(nComp, colLength, d_ax, eps_c, eps_p, kf, Rp, Rc, Dp, c0, cp0, q0, cIn, exactInt, polyDeg, nCells, polyDegPore, ConvDispOpInstance, PoreOpInstance, bindStride, adsStride, unitStride, idx, idx_p, idx_q, Fc, Fp, Fjac, Jr, invRi, cpp, RHS_q, qq, solution_outlet, solution_times, bind, checkReactions, reaction_liquid, reaction_particle, reaction_solid)
	end
end
