...
mutable struct cstr
	...
	function cstr(; nComp, V, c0 = 0)
		...
		reaction_cstr = no_reaction()


		new(nComp, V, cIn, c0, unitStride, solution_outlet, solution_times, checkReactions, reaction_cstr)
	end
end
...
function compute!(RHS, RHS_q, cpp, qq, x, m::cstr, t, section, sink, switches, idx_units) 
	...
	compute_reactions!(RHS, RHS_q, cpp, qq, x, idx_units[sink], m, t, m.checkReactions)
